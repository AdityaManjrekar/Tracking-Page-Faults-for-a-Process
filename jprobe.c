...
static int __init jprobe_init(void)
{
  ...
  {
    printk(KERN_INFO "Proc Entry Failed\n");
  	return -1;
  }
  printk(KERN_INFO "Planted jprobe at %p, handler addr %p and /Proc - Probe registered Succesfully\n",my_jprobe.kp.addr, my_jprobe.entry);
  return 0;
}
...
static void __exit jprobe_exit(void)
{
  ...
  printk(KERN_INFO "jprobe at %p unregistered and /Proc - Probe unregistered\n", my_jprobe.kp.addr);
}

 int handle_mm_fault(struct mm_struct *mm, struct vm_area_struct *vma,unsigned long address, unsigned int flags)
 {
  struct timespec xtime;//created a timespec variable to hold the current time
